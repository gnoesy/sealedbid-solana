...
pub mod sealedbid {
    ...
    /// Submit two encrypted sealed bids for MXE comparison.
    /// Neither bid amount is revealed — only the MXE sees plaintext.
    pub fn compare_bids(
        ctx: Context<CompareBids>,
        computation_offset: u64,
        encrypted_bid1: [u8; 32],
        encrypted_bid2: [u8; 32],
        pubkey: [u8; 32],
        nonce: u128,
    ) -> Result<()> {
        ...
        let args = ArgBuilder::new()
            ...
            .encrypted_u8(encrypted_bid1)
            .encrypted_u8(encrypted_bid2)
            .build();
