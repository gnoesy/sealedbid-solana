...
async function main() {
  ...
  const idl = JSON.parse(fs.readFileSync("target/idl/sealedbid.json", "utf-8"));
  ...
  try {
    ...
    console.log("init_compare_bids_comp_def sig:", sig);
  } catch (e: any) {
  ...
  await uploadCircuit(provider, "compare_bids", program.programId, rawCircuit, true, 500,
